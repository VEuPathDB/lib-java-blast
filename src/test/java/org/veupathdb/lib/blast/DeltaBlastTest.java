...
class DeltaBlastTest
{
  ...
  class DomainInclusionEThreshold1
  {
    @Test
    @DisplayName("serialization")
    void test1() throws Exception {
      var tgt = new DeltaBlast();
      tgt.setDomainInclusionEThreshold(123.0);

      assertEquals("{\"-domain_inclusion_ethresh\":123.0}", json.writeValueAsString(tgt));
    }

    @Test
    @DisplayName("deserialization")
    void test2() throws Exception {
      var raw = "{\"-domain_inclusion_ethresh\":123.0}";
      var tgt = json.readValue(raw, DeltaBlast.class);

      assertEquals(123, tgt.getDomainInclusionEThreshold());
    }
  }
  ...
  class RPSDBFile1
  {
    ...
    void test1() throws Exception {
      ...
      assertEquals("{\"-rpsdb\":\"hi\"}", json.writeValueAsString(tgt));
    }
    ...
    void test2() throws Exception {
      ...
      assertEquals("hi", tgt.getRPSDBFile());
    }
  }

  @Nested
  @DisplayName(Flag.ShowDomainHits)
  class ShowDomainHits1
  {
    @Test
    @DisplayName("serialization")
    void test1() throws Exception {
      var tgt = new DeltaBlast();
      tgt.setShowDomainHits(true);

      assertEquals("{\"-show_domain_hits\":true}", json.writeValueAsString(tgt));
    }

    @Test
    @DisplayName("deserialization")
    void test2() throws Exception {
      var raw = "{\"-show_domain_hits\":true}";
      var tgt = json.readValue(raw, DeltaBlast.class);

      assertEquals(true, tgt.getShowDomainHits());
    }
  }
}