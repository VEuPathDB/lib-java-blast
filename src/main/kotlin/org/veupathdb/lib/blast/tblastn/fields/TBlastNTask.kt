 ...
 * Default Value
 */
private val Def = TBlastNTaskValue.TBlastN
 ...
 */
@JvmInline
value class TBlastNTask(val value: TBlastNTaskValue = Def) : BlastField {
...
/**
 * Parse [TBlastNTaskValue] from string.
 */
private inline fun parse(value: String) =
  when (value.lowercase()) {
    "tblastn"      -> TBlastNTaskValue.TBlastN
    "tblastn-fast" -> TBlastNTaskValue.TBlastNFast
    else           -> throw IllegalArgumentException("Invalid $FlagTask value: $value")
  }


enum class TBlastNTaskValue {
  TBlastN,
  TBlastNFast;
